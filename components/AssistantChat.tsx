...
export default function AssistantChat() {
  ...
  // Escuchar el final de la animación del loader de gala
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      window.addEventListener("first-load-complete", handleComplete);
      
      // Fallback de 6.5 segundos para evitar apariciones prematuras en conexiones lentas o subpáginas
      const timer = setTimeout(() => {
        setShouldShow(true);
      }, 6500);

      return () => {
        window.removeEventListener("first-load-complete", handleComplete);
        clearTimeout(timer);
      };
    }
  }, []);
  ...
  const processResponse = (text: string): string => {
    ...
    if (raw.includes("victoria") || raw.includes("repostera") || raw.includes("creadora") || raw.includes("quien es") || raw.includes("quien hace")) {
      return "**Victoria** es la mente y las manos creadoras de Caracas Mini Dulces. 🧑‍🍳✨\n\nElla es una apasionada de la repostería artística y detallista. Su filosofía se basa en el **modelado 100% manual**, asegurando que cada figura o rosa de azúcar sea una pieza única con alma, combinando una presentación visual deslumbrante y detallista con texturas y sabores gourmet extraordinarios.";
    }
